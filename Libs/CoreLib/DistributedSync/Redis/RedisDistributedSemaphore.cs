...
/// <summary>
/// Распределённый семафор на Redis через Sorted Set + Lua
/// Ключ: ZSET с элементами = lockId, score = expiresAtMillis
/// </summary>
...
";
    ...
    public RedisDistributedSemaphore(IDatabase db, string name, int maxCount, TimeSpan lease)
    {
        ...
        _key = (RedisKey)(name ?? throw new ArgumentNullException(nameof(name)));
        if (maxCount < 1)
        {
            throw new ArgumentOutOfRangeException(nameof(maxCount));
        }
        _maxCount = maxCount;
        _lease = lease <= TimeSpan.Zero ? TimeSpan.FromSeconds(30) : lease;
    ...
    {
        var result = await _db.ScriptEvaluateAsync(PurgeAndCountScriptText, [_key], values: []).ConfigureAwait(false);

        // RedisResult приходит как Int64
    ...
    public async ValueTask<IDistributedSemaphoreHandle?> TryAcquireAsync(TimeSpan timeout, CancellationToken ct = default)
    {
        ...
        {
            var result = await _db.ScriptEvaluateAsync(AcquireScriptText, [_key], values: [_maxCount, (int)_lease.TotalMilliseconds, lockId, (int)setTtl.TotalMilliseconds]).ConfigureAwait(false);
            ...
            // небольшой джиттер против конвоя
            var sleepMs = Random.Shared.Next(10, 40);
            try
            {
    ...
    public async ValueTask<IDistributedSemaphoreHandle> AcquireAsync(TimeSpan timeout, CancellationToken ct = default)
    {
        ...
        if (handle is null)
        {
            throw new TimeoutException($"Semaphore '{Name}' acquire timed out");
        }
        return handle;
    }
}
